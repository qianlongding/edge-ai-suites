...
This guide provides step-by-step instructions for running the benchmarking script to evaluate the performance of the Loitering Detection application. The script can help you determine the maximum number of concurrent video streams your system can handle while meeting a specific performance target (e.g., frames per second).
...
*   A successful deployment of the Loitering Detection application using Helm, as described in the [How to Deploy with Helm](./how-to-deploy-with-helm.md) guide.
...
*   `pipeline`: The name of the pipeline to execute (e.g., `object_tracking_benchmarking`).
...
```bash
./benchmark_start.sh -p loitering-detection/benchmark_cpu_payload.json -n 4
```

*   `-p loitering-detection/benchmark_cpu_payload.json`: Specifies the path to your payload configuration file.
...
```bash
./benchmark_start.sh -p loitering-detection/benchmark_cpu_payload.json -t 28.5
```
...
In this guide, you learned how to use the `benchmark_start.sh` script to run performance tests on your Loitering Detection application. You can now measure performance for a fixed number of streams or automatically determine the maximum stream density your system can support.
