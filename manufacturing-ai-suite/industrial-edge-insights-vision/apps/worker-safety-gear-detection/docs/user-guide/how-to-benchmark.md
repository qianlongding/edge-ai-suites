...
1. Test the app's best performance with GPU acceleration:
   ```bash
   ./benchmark_start.sh -p apps/worker-safety-gear-detection/gpu_payload.json -l 1 -u 10 -t 25.0 -i 30
   ```

2. Test maximum GPU performance for pure throughput:
   ```bash
   ./benchmark_start.sh -p apps/worker-safety-gear-detection/benchmark_gpu_payload.json -l 1 -u 20 -t 28.5 -i 60
   ```
